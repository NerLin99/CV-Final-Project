...
# Select modified model
# from spann3r.model_modified import Spann3R
...
def main(args):
    ...
    #     print(f"Frame {j}: pts3d shape = {pred.shape}")
    ...
    valid_points = pts_all[conf_sig_all > args.conf_thresh].reshape(-1, 3)
    print(f"Number of valid points: {valid_points.shape[0]}")

    if valid_points.shape[0] == 0:
        print("No valid points to save. Check conf_thresh or model output.")


    try:
        pcd = o3d.geometry.PointCloud()
        valid_pts = pts_all[conf_sig_all > args.conf_thresh].reshape(-1, 3)
        valid_pts = valid_pts.astype(np.float64)
        print("valid_pts shape:", valid_pts.shape)
        pcd.points = o3d.utility.Vector3dVector(valid_pts)
        valid_colors = images_all[conf_sig_all > args.conf_thresh].reshape(-1, 3)
    ...
    if args.vis:
        try:
            ...
            print(f"Rendering frames for {demo_name}")
