...
class SpatialMemory():
    def __init__(self, norm_q, norm_k, norm_v, mem_dropout=None, 
                 ...
                 soft_prune_threshold=1e-3,              # Added: Soft prune threshold
                 low_priority_retention_frames=10):       # Added: low-priority zone retention time (frames)
    ...
    def init_mem(self):
        ...
        self.low_priority_age = None  # Used to record how many frames stayed in low priority
    ...
    def multi_stage_memory_prune(self):
        ...
        1. Soft delete entries below `soft_prune_threshold` into low-priority storage.
        2. Recover entries from low-priority storage if they meet recovery criteria.
        3. Hard delete entries in low-priority storage that are still unimportant after a certain number of frames.
        4. Perform top-K pruning on remaining entries in primary storage.
        """
        # Update the age of the low-priority zone
        self.update_low_priority_age()

        # Stage 1: Soft prune
        weights = self.mem_attn / (self.mem_count + 1e-8)
        soft_prune_mask = (weights < self.soft_prune_threshold).squeeze(-1)  # [B, T]

        if torch.any(soft_prune_mask):
            self.move_to_low_priority(soft_prune_mask)

        # Stage 2: Recover from low-priority memory
        self.recover_from_low_priority()

        # Stage 3: Hard prune
        self.hard_delete_from_low_priority()

        # Stage 4: Final top-K pruning
    ...
    def move_to_low_priority(self, mask):
        ...
        # mask: [B, T]
        # Move entries with True in mask from main storage to low priority storage area

        # Collecting low-priority entries per batch
        def gather_selected(tensor, mask):
            # tensor: [B, T, C]
            # mask: [B, T]
            # return: [B, T_selected, C]
        ...
        # Add these entries to the low-priority store
        self.add_to_low_priority(sel_k, sel_v, sel_attn, sel_count, sel_pts, sel_imgs)

        # Remove these entries from primary storage
        keep_mask = ~mask
    ...
    def add_to_low_priority(self, k, v, attn, count, pts, imgs):
        # Append selected entries to low_priority_mem
        def concat_or_init(orig, new):
            ...
            if orig.shape[-1] != new.shape[-1]:
                raise ValueError(f"Dimension mismatch in C: orig.shape[-1]={orig.shape[-1]}, new.shape[-1]={new.shape[-1]}")
    ...
    def filter_main_memory(self, keep_mask):
        # keep_mask: [B, T]
        # Keep primary storage entries according to keep_mask
    ...
    def hard_delete_from_low_priority(self):
        # Perform hard deletion of entries that stay in low priority for more than low_priority_retention_frames and remain useless (low attn)
        print('Start hard prune')
        if self.low_priority_mem_k is None:
            return

        # Determine if still useless based on attn
        weights_lp = self.low_priority_mem_attn / (self.low_priority_mem_count + 1e-8)
        hard_delete_mask = (self.low_priority_age > self.low_priority_retention_frames) & (weights_lp < self.soft_prune_threshold)  # [B, T_lp, 1]

        if torch.any(hard_delete_mask):
            keep_mask = ~hard_delete_mask.squeeze(-1)  # [B, T_lp]
            self.filter_low_priority_memory(keep_mask)


    def filter_low_priority_memory(self, keep_mask):
        # keep_mask: [B, T_lp]
        # low_priority_mem_k: [B, T_lp, C]
        # low_priority_mem_attn: [B, T_lp, 1]
        
        print(f"keep_mask shape before processing: {keep_mask.shape}")
        if keep_mask.dim() != 2:
            raise ValueError(f"keep_mask should have 2 dimensions [B, T_lp], but got {keep_mask.dim()} dimensions.")

        if keep_mask.shape[1] != self.low_priority_mem_k.shape[1]:
            raise ValueError(
                f"Shape mismatch: keep_mask.shape={keep_mask.shape} does not match "
                f"low_priority_mem_k.shape[1]={self.low_priority_mem_k.shape[1]}"
            )

        print(f"keep_mask shape after processing: {keep_mask.shape}")
        print(f"low_priority_mem_k shape: {self.low_priority_mem_k.shape}")

        # 定义一个辅助函数，根据张量的最后一个维度决定掩码的形状
    ...
    def final_topk_prune(self):
        # Final retention of the current primary storage according to the original top_k policy
        if self.mem_k is None:
            return
        weights = self.mem_attn / (self.mem_count+1e-8)
    ...
    def recover_from_low_priority(self):
        """
        Recover entries from low-priority memory to main memory based on certain conditions,
        such as high attention weights or recent activity.
        """
        if self.low_priority_mem_k is None:
            print("Low-priority memory is empty, nothing to recover.")
            return

        # Calculate weights (attn / count) to decide which entries to recover
        weights_lp = self.low_priority_mem_attn / (self.low_priority_mem_count + 1e-8)
        recovery_mask = weights_lp > self.soft_prune_threshold  # [B, T_lp, 1]

        if torch.any(recovery_mask):
            print(f"Recovering {recovery_mask.sum().item()} entries from low-priority memory.")
    ...
    def recover_to_main_memory(self, recovery_mask):
        """
        Move selected entries from low-priority memory to main memory.
        """
        # recovery_mask: [B, T_lp, 1]
        # We need to recover per batch
        ...
        for b in range(B):
            mask_b = recovery_mask[b, :, 0]  # [T_lp]
            if torch.any(mask_b):
                k_b = self.low_priority_mem_k[b, mask_b, :]  # [T_recovered, C]
                v_b = self.low_priority_mem_v[b, mask_b, :]  # [T_recovered, C]
                attn_b = self.low_priority_mem_attn[b, mask_b, :]  # [T_recovered, 1]
                count_b = self.low_priority_mem_count[b, mask_b, :]  # [T_recovered, 1]
                ...
                if self.low_priority_mem_pts is not None:
                    pts_b = self.low_priority_mem_pts[b, mask_b, :, :]  # [T_recovered, 256, 3]
                    recovered_pts.append(pts_b)
                if self.low_priority_mem_imgs is not None:
                    imgs_b = self.low_priority_mem_imgs[b, mask_b, :, :]  # [T_recovered, 256, 3]
                    recovered_imgs.append(imgs_b)

        # Stack recovered entries per batch
        if recovered_k:
            recovered_k = torch.stack(recovered_k, dim=0)  # [B, T_recovered, C]
